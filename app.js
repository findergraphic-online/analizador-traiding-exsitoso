...
// Backtest + Real Mode | Charts Library
// =============================================
...
let currentMode = 'backtest'; // 'backtest' | 'real'
...
// =================== STORAGE ===================
function loadDB() {
  try { const d = localStorage.getItem('tj_pro_db'); if (d) DB = JSON.parse(d); } catch(e) {}
}
function saveDB() { localStorage.setItem('tj_pro_db', JSON.stringify(DB)); }
...
function calcMetrics(trades) {
  ...
  return { total, winRate:wr, trades:exec.length, wins:wins.length, pf, dd, expectancy:exp, gross_profit:gp, gross_loss:gl, commissions };
}
...
function showPage(p) {
  ...
  const titles = {
    ...
    'charts-library':'Biblioteca de Charts', weekly:'Análisis Semanal',
...
function renderDashboard() {
  ...
  return `
${isReal ? `<div style="background:var(--blue-dim);border:1px solid rgba(41,121,255,.3);border-radius:var(--radius2);padding:10px 16px;margin-bottom:16px;display:flex;gap:16px;align-items:center;font-size:13px">
...
function initDashboardCharts() {
  ...
  sorted.forEach(t=>{ if(t.resultado!=='Sin Entrada'){ eq+=parseFloat(t.pnlUSD)||0; eL.push(t.date); eD.push(parseFloat(eq.toFixed(2))); }});
  ...
  newBarChart('ch-daily',dk,dk.map(k=>parseFloat(dm[k].toFixed(2))),dk.map(k=>dm[k]>=0?'#00e676':'#ff1744'));
  // dist
...
<tbody>${trades.map(t=>`<tr>
  ...
  <td>${(t.img1||t.img2)?`<button class="btn btn-secondary btn-sm" onclick="viewTradeImages('${t.id}')">🖼️ ${[t.img1,t.img2].filter(Boolean).length}</button>`:'<span style="color:var(--text3)">—</span>'}</td>
  ${mini ? '' : `<td style="white-space:nowrap"><button class="btn btn-secondary btn-sm" onclick="openTradeModal('${t.id}')">✏️</button> <button class="btn btn-danger btn-sm" onclick="deleteTrade('${t.id}')">🗑</button></td>`}
</tr>`).join('')}</tbody></table>`;
}
...
<div class="card" style="max-width:920px">
  ...
  <div class="form-grid">
    ...
    <div class="form-group"><label>Temporalidad</label><input type="text" id="f-tf" placeholder="1M, 5M, 15M, 1H, 4H, D1..."></div>
    ...
    <div class="form-group form-full"><label>Notas / Análisis</label><textarea id="f-notes" rows="3" placeholder="Describe el setup, motivo de entrada, gestión, lecciones aprendidas..."></textarea></div>
    <div class="form-group">
      <label>📷 Captura Inicial (antes del trade)</label>
      <div class="img-upload-area" onclick="document.getElementById('f-img1').click()">
        ...
        <span>Click para subir imagen inicial</span>
    ...
    <div class="form-group">
      <label>📷 Captura Final (resultado del trade)</label>
      <div class="img-upload-area" onclick="document.getElementById('f-img2').click()">
        ...
        <span style="font-size:20px">📸</span>
        <span>Click para subir imagen final</span>
...
${t&&t.img1?`<div class="form-group"><label>Captura Inicial (guardada)</label><img src="${t.img1}" class="img-preview-thumb" onclick="openImgModal(this.src,'Captura Inicial')"></div>`:''}
${t&&t.img2?`<div class="form-group"><label>Captura Final (guardada)</label><img src="${t.img2}" class="img-preview-thumb" onclick="openImgModal(this.src,'Captura Final')"></div>`:''}
</div>`;
...
function renderChartsLibrary() {
  ...
  const imgs = [];
  filtered.forEach(t => {
    if (t.img1) imgs.push({ src:t.img1, trade:t, label:'Inicial', caption:`${fmtDate(t.date)} · ${stratName(t.strategyId)} · ${t.asset||''} ${t.tf||''} · ${t.resultado}` });
    if (t.img2) imgs.push({ src:t.img2, trade:t, label:'Final',   caption:`${fmtDate(t.date)} · ${stratName(t.strategyId)} · ${t.asset||''} ${t.tf||''} · Resultado` });
  });
...
<div class="chart-card">
  <img src="${img.src}" class="chart-card-img" alt="${img.caption}"
    onclick="openImgModal('${img.src}','${img.caption.replace(/'/g,"\\'")}','${img.trade.notes||''}')"
    onerror="this.style.display='none'">
  <div class="chart-card-body">
    <div style="font-size:13px;font-weight:600">${fmtDate(img.trade.date)} · ${img.trade.asset||'—'}</div>
    <div class="chart-card-meta">
      ...
      <span class="tag tag-gray">${img.label}</span>
    ...
    ${img.trade.notes?`<div style="font-size:12px;color:var(--text2);margin-top:6px;line-height:1.5">${img.trade.notes.slice(0,80)}${img.trade.notes.length>80?'…':''}</div>`:''}
  </div>
</div>`).join('')}
</div>`;
}
...
function openImgModal(src, caption, notes) {
  ...
  document.getElementById('modal-img-caption').innerHTML =
    `<span>${caption||''}</span>` + (notes ? `<br><span style="font-size:11px;color:var(--text3)">${notes}</span>` : '');
...
function viewTradeImages(tradeId) {
  ...
  if (!t) return;
  if (t.img1) openImgModal(t.img1, `${fmtDate(t.date)} · ${stratName(t.strategyId)} · Captura Inicial`, t.notes);
  else if (t.img2) openImgModal(t.img2, `${fmtDate(t.date)} · ${stratName(t.strategyId)} · Captura Final`, t.notes);
}
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;max-width:800px">
  <div class="card">
    ...
    <div class="form-grid">
      <div class="form-group form-full"><label>Capital Base (USD)</label><input type="number" id="s-capital" value="${DB.settings.capital}" oninput="updateRiskPct()"></div>
...
<div class="card" style="max-width:800px;margin-top:16px">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    ...
    <div>
      ...
      <div style="font-size:11px;color:var(--text3);margin-top:8px">CSV de MT4/MT5: columnas Time, Symbol, Type, Profit (orden flexible)</div>
    </div>
  </div>
  <div class="divider"></div>
...
function saveSettings() {
  ...
  DB.settings.accountType = document.getElementById('s-actype').value;
  saveDB(); alert('✅ Configuración guardada');
}

function clearAll() {
  if (!confirm('¿Eliminar TODOS los datos? Esta acción no se puede deshacer.')) return;
  DB = { strategies:[], trades:[], realTrades:[], settings:{capital:56000,riskPerTrade:560,currency:'USD',broker:'',accountNumber:'',accountType:'Demo'} };
  saveDB(); renderCurrentPage();
}
...
function exportCSV() {
  const trades = filteredTrades();
  const isReal = currentMode === 'real';
  const headers = ['Modo','Fecha','Estrategia','Activo','TF','Semana','Mes','Sesión','Tendencia','Resultado','P&L USD','P&L %','Capital','Riesgo', isReal?'Comisión':'', 'Notas'];
  ...
  const csv = [headers.filter(Boolean).join(','), ...rows.map(r=>r.filter((_,i)=>isReal||i!==14).join(','))].join('\n');
...
function importCSV(e) {
  ...
  r.onload = ev => {
    ...
    for (let i=1; i<lines.length; i++) {
      ...
      // MT4/MT5 format: Time, Symbol, Type, Volume, Price, S/L, T/P, Close Price, Commission, Swap, Profit
...
function seedDemo() {
  DB.strategies=[
    {id:'s1',name:'ICT Concepts',color:'#00e676',description:'Inner Circle Trader'},
  ...
  const now=new Date();
  const add=(d,sId,res,pnl,pct,sess,mode='backtest',comm=0)=>{
    const dt=new Date(now); dt.setDate(now.getDate()-d);
    const ds=dt.toISOString().slice(0,10);
    const t={id:uid(),date:ds,strategyId:sId,asset:'EURUSD',tf:'1H',session:sess,tendencia:res==='Positivo'?'Alcista':res==='Negativo'?'Bajista':'Lateral',resultado:res,pnlUSD:pnl,pnlPct:pct,capital:56000,risk:560,commission:comm,week:getWeek(ds),month:getMonth(ds),mode,notes:'',img1:'',img2:''};
    if(mode==='real') DB.realTrades.push(t); else DB.trades.push(t);
  };
  add(1,'s1','Positivo',672,1.2,'London'); add(2,'s1','Negativo',-560,-1,'New York');
  add(3,'s2','Positivo',896,1.6,'London'); add(5,'s2','Sin Entrada',0,0,'Asia');
  add(6,'s3','Positivo',1120,2,'New York'); add(7,'s1','Negativo',-280,-0.5,'London');
  add(8,'s3','Positivo',448,0.8,'Asia'); add(10,'s2','Positivo',784,1.4,'London');
  add(12,'s1','Negativo',-560,-1,'New York'); add(14,'s3','Positivo',1344,2.4,'London');
  add(17,'s1','Positivo',560,1,'New York'); add(20,'s2','Negativo',-280,-0.5,'London');
  add(22,'s3','Positivo',672,1.2,'Asia'); add(25,'s1','Positivo',896,1.6,'New York');
  // demo real trades
  add(3,'s1','Positivo',340,0.6,'London','real',7.5); add(5,'s2','Negativo',-280,-0.5,'New York','real',7.5);
  add(7,'s1','Positivo',420,0.75,'London','real',7.5); add(10,'s3','Sin Entrada',0,0,'Asia','real',0);
  add(12,'s2','Positivo',560,1,'New York','real',7.5); add(15,'s1','Negativo',-280,-0.5,'London','real',7.5);
